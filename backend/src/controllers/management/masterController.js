...
const ModelData = require("../../models/management/masterModel");
...
exports.getComboNasabah = async (req, res, next) => {
  try {
    const { roles_id, id: userId } = req.user;

    const [rows] = await ModelData.getComboNasabah(roles_id, userId);
    res.json(rows);
  } catch (err) {
    next(err);
  }
